...
function addRevertibleEventListener(targetElement, eventType, eventCallback, options) {
  ...
  if (!targetElementListenersName) {
    // First event listener for this target element:
  ...
  if (!eventTypeListeners) {
    // No actual event type listeners yet:
...
export function makeReordable(containerEl, dropCallBack) {
  // Container events:
  addRevertibleEventListener(
    containerEl,
    'drop',
    () => {
      // Record successful drop (to make distinction from 'cancel'):
      dragState.dropped = true;
    },
    { passive: true },
  );
  addRevertibleEventListener(containerEl, 'dragover', (ev) => {
    // Enable as drop-target:
    ev.preventDefault();
  });
  addRevertibleEventListener(containerEl, 'dragend', (ev) => {
    // Finish operation: cancel or finalize
    ev.preventDefault();
...
function checkCurrentlyDraggable(elem) {
  const dragElem = elem.closest('[draggable]'); // Look only to element itself and up to parent elements
  ...
  return (
    window.getComputedStyle(dragElem).display !== 'none' && window.getComputedStyle(dragElem).visibility !== 'hidden'
  );
}
...
function moveAfter(parent, elToMove, elToMoveAfter) {
  const nextEl = elToMoveAfter.nextElementSibling;
  if (nextEl) {
