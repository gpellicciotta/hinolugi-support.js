...
export function start(canvasEl, options) {
  ...
  if (options['svg-path-id']) {
    options.svgPathId = options['svg-path-id'];
  }
...
function setCanvasSizes(canvasEl) {
  ...
  const canvasWidth = +getComputedStyle(canvasEl).getPropertyValue('width').slice(0, -2);
  const canvasHeight = +getComputedStyle(canvasEl).getPropertyValue('height').slice(0, -2);
...
function step(fireworksInfo) {
  ...
  const interval = 1000 / fireworksInfo.frequency;
...
class FireworkBox {
  ...
  startNewFirework(target) {
    ...
    const xVel = (xTarget - xOrig) / t;
    const yVel = -Math.sqrt(2 * GRAVITY.y * (this._height - yTarget));

    const initialVelocity = new Vector(xVel, yVel);
...
class Firework {
  ...
  step() {
    ...
    if (!this._exploded) {
      ...
      // If already exploded into multiple particles
      for (let i = this._particles.length - 1; i >= 0; i--) {
        const p = this._particles[i];
        p.step();
        if (p.dead) {
  ...
  explode(origin) {
    ...
    switch (shape) {
      ...
      case 'stars': {
        const STAR_SVG_PATH =
          'm92.01591,-64.30286l-40.05686,64.68039l46.72933,60.03727l-73.89295,-18.10898l-42.65866,62.99478l-5.61151,-75.87238l-73.09385,-21.10436l70.42485,-28.7827l-2.5158,-76.03799l49.13646,58.08368l71.539,-25.88971l-40.05686,64.68039z';
  ...
  explodeAsNormal(origin) {
    const partCount = 180;
    const angleIncr = (Math.PI * 2) / partCount;
    for (let i = 0, a = 0; i < partCount; i++, a += angleIncr) {
      const aa = Math.random(a);
      const m = 0.1 + 1.5 * aa;
      const v = vectorFromPolar(a, m).plus(this._initialVelocity);
      const p = new Particle(origin, v, this._hue);
  ...
  explodeAsHeart(origin) {
    ...
    for (let i = 0, a = 0; i < partCount; i++, a += angleIncr) {
      ...
      const y = 0 - (13 * Math.cos(a) - 5 * Math.cos(2 * a) - 2 * Math.cos(3 * a) - Math.cos(4 * a));
      const m = 0.05;
      const v = vectorFromCartesian(x, y).multiply(m);
      const p = new Particle(origin, v, this._hue);
      this._particles.push(p);
    }
  }
  ...
  explodeAsDonut(origin) {
    ...
    for (let i = 0, a = 0; i < partCount; i++, a += angleIncr) {
      const m = 0.8;
      const v = vectorFromPolar(a, m);
      const p1 = new Particle(origin, v, this._hue);
      this._particles.push(p1);
      const p2 = new Particle(origin, v.multiply(0.8), this._hue);
  ...
  explodeAsTwinkle(origin) {
    ...
    for (let i = 0; i < 3 * arms; i++) {
      const r = radiuses[i % 3];
      const v = vectorFromPolar(i * angle, r);
      const p = new Particle(origin, v, this._hue);
      this._particles.push(p);
    }
  }
  ...
  explodeAsSvgPath(origin, svgPathEl) {
    const partCount = 180;
    const pathLength = Math.floor(svgPathEl.getTotalLength());
    let max = 0;
    const vectors = [];
    for (let i = 0; i < partCount; i++) {
      const pt = svgPathEl.getPointAtLength((pathLength * i) / partCount);
...
class Particle {
  ...
  render(ctx) {
    ...
    const lastTrail = this._trails[this._trails.length - 1];
